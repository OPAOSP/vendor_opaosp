...
func init() {
	android.RegisterModuleType("opaosp_generator", GeneratorFactory)
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = opaospExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = opaospExpandVariables(ctx, dep_file)
	...
	cmd := opaospExpandVariables(ctx, String(g.properties.Cmd))
